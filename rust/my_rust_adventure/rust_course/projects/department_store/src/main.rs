...
fn main() {
    // hashmap will have strings as keys and vector of strings as values
    ...
    // use of for loop and match to determine input and command functionality
    for line in stdin.lock().lines() {
        let input = line.expect("error: unable to read user input");
        match Command::from_input(&input) {
            /*
            or_default is just a convenience, does the same
            as or_insert_with(Vec::default)
            */
